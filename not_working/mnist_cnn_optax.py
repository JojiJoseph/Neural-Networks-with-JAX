# This is to test JAX with optax, but without flax module (except for the train_state)
# As of now, this test is a failure. It doesn't work for non flax modules.
import numpy as np
...
state = train_state.TrainState.create(
    apply_fn=vmap(model, in_axes=[None, 0]),
    params=params,
    tx=optax.adam(1e-3),
)
...
for epoch in range(20):
    for i in range(0, len(X_train), 64):
        ...
        # grads = jax.grad(loss_fn, argnums=(0, ))(state.params, X_batch, y_batch)
        def loss_fn(params, X, y):
            y_hat = state.apply_fn(params, X)
            return categorical_cross_entropy(y, y_hat, one_hot=False).mean()
        # out = state.apply_fn(state.params, X_batch)
        grads = jax.grad(loss_fn, argnums=(0, ))(state.params, X_batch, y_batch)
        # categorical_cross_entropy(y, y_hat, one_hot=False) / float(y.shape[0])
